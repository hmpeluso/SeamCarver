 ...
 * blank image of a given dimension. Includes methods for displaying the image
 * in a window on the screen or saving to a file.
 *
...
public final class Picture implements ActionListener {
    ...
    private void validateRowIndex(int row) {
        if (row < 0 || row >= height())
            throw new IllegalArgumentException(
                    "row index must be between 0 and " + (height() - 1) + ": "
                            + row);
    }
    ...
    private void validateColumnIndex(int col) {
        if (col < 0 || col >= width())
            throw new IllegalArgumentException(
                    "column index must be between 0 and " + (width() - 1) + ": "
                            + col);
    }
